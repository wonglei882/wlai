...
async def generate_storyboard(req: StoryboardGenerateRequest, request: Request):
    """从文本自动生成分镜表。"""
...
async def get_storyboard(storyboard_id: str, request: Request):
    """获取分镜表详情（含镜头列表）。"""
    ...
    try:
        ...
        if not sb:
            raise HTTPException(status_code=404, detail='分镜表不存在')
...
async def update_storyboard(storyboard_id: str, req: StoryboardUpdateRequest, request: Request):
    """编辑分镜表。"""
    db, _ = await _get_db(request)
    try:
        result = await db.execute(
...
async def confirm_storyboard(storyboard_id: str, request: Request):
    """确认分镜表。"""
    db, _ = await _get_db(request)
    try:
        result = await db.execute(
...
@router.get('/shots')
async def list_shots(storyboard_id: str, request: Request):
    """列出分镜表下所有镜头。"""
    db, _ = await _get_db(request)
    try:
        result = await db.execute(
...
async def update_shot_status(shot_id: str, req: ShotStatusRequest, request: Request):
    """镜头状态流转。"""
    db, _ = await _get_db(request)
    try:
        result = await db.execute(
...
async def compile_shot_prompt(shot_id: str, req: ShotCompileRequest, request: Request):
    """编译单镜头提示词。"""
    db, _ = await _get_db(request)
    try:
        # 获取镜头数据
...
@router.post('/assets')
async def create_asset(req: AssetCreateRequest, request: Request):
    """上传/注册素材。"""
    db, user_id = await _get_db(request)
    try:
        # 计算版本号
...
@router.get('/assets')
async def list_assets(shot_id: str, request: Request):
    """列出镜头下所有素材。"""
    db, _ = await _get_db(request)
    try:
        result = await db.execute(
...
@router.post('/reviews')
async def create_review(req: ReviewCreateRequest, request: Request):
    """创建审核点。"""
    db, user_id = await _get_db(request)
    try:
...
async def update_review(review_id: str, req: ReviewUpdateRequest, request: Request):
    """提交审核结果。"""
    db, _ = await _get_db(request)
    try:
        result = await db.execute(
        ...
        if not review:
            raise HTTPException(status_code=404, detail='审核点不存在')
...
@router.get('/reviews')
async def list_reviews(project_id: str, status: str = 'pending', request: Request = None):
    """列出待审核项。"""
    db, _ = await _get_db(request)
    try:
        result = await db.execute(
