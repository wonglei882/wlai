...
class ShotGenerateRequest(BaseModel):
    platform: str = Field(default='midjourney', description='提示词平台')
    provider: str = Field(default='mock', description='生成器: mock（后续接入即梦/可灵等）')
    asset_type: str = Field(default='image', description='image / video / voice')


@router.post('/shots/{shot_id}/generate')
async def generate_shot_asset(
    shot_id: str,
    ...
    当前使用 Mock 生成器产出占位素材；接入真实平台后同一端点自动切换。
    """
    from app.services.generators import (
        get_image_generator, get_video_generator, get_voice_generator,
    )

    shot_result = await db.execute(select(Shot).where(Shot.id == shot_id))
    shot = shot_result.scalar_one_or_none()
    if not shot:
        raise NotFoundError('镜头', shot_id)
    ...
    generator_map = {
        ...
        'voice': get_voice_generator,
    }
    generator = generator_map[req.asset_type](req.provider)
