    ...
    async def chat_completion(
        ...
        return {
            ...
            'usage': {
                ...
                'total_tokens': ((getattr(usage, 'input_tokens', 0) or 0) + (getattr(usage, 'output_tokens', 0) or 0)) if usage else None,
            },
        }
                    ...
                    async for chunk in stream:
                        ...
                        if chunk.type == 'text_delta':
                            ...
                            if not tool_calls or tool_calls[-1].get('id') != chunk.id:
                                tool_calls.append({'id': chunk.id, 'type': 'function', 'function': {'name': chunk.name, 'arguments': ''}})
                            # 追加参数
