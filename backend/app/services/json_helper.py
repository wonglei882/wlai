...
def _is_content_quote(text: str, pos: int) -> bool:
    ...
    if ch == '\n' or ch == '\r':
        ...
        return not (text[k] == '"' or text[k] in ('}', ']'))
    ...
    if ch == ',':
        ...
        if text[k] in ('\n', '\r'):
            ...
            if k2 >= len(text):
                return False
            return not (text[k2] == '"' or text[k2] in ('}', ']'))
...
def clean_json_response(text: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f'[json_helper] clean_json_response 失败: {e}')
...
def parse_json(text: str) -> dict | list:
    ...
    try:
        ...
        logger.warning(f'[json_helper] parse_json 失败: {e}')
...
def loads_json(text: str) -> Any:
    ...
    try:
        ...
        logger.warning(f'[json_helper] loads_json 失败: {e}')
...
def safe_json_loads(text: Any, default: Any = None) -> Any:
    """容错 JSON 解析：None/空/解析失败都返回 default。

    用于 DB 字段读取、LLM 输出等不可信输入，消除
    `json.loads(x) if x else {}` + `except Exception: {}` 的逐处重复。

    Args:
        text: 待解析文本（允许 None / 空字符串 / 非 str）
        default: 解析失败或输入为空时返回的默认值
    ...
    try:
        ...
        logger.debug(f'[json_helper] safe_json_loads 失败，返回默认 {type(default).__name__}: {e}')
        return default


def safe_int(value: Any, default: int = 0) -> int:
    """安全 int 转换：None/空/非数字都返回 default。

    用于不可信输入（LLM 工具调用参数、HTTP 查询参数），
    避免 ValueError 冒泡成 500。替代 `safe_int(params.get(x, 0), 0)` 裸调。

    Args:
        value: 待转换值（允许 None / str / float / bool）
        default: 转换失败时的默认值
...
def safe_float(value: Any, default: float = 0.0) -> float:
    """安全 float 转换：None/空/非数字都返回 default。

    用于不可信输入（LLM 工具调用参数、HTTP 查询参数），
    避免 ValueError 冒泡成 500。替代 `safe_float(params.get(x, 0.5), 0.5)` 裸调。

    Args:
        value: 待转换值（允许 None / str / int / bool）
        default: 转换失败时的默认值
