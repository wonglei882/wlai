...
class PlotAnalysis(Base):
    ...
    # ========== B方案新增质量指标 ==========
    causal_density = Column(Float, comment='因果链密度（每千字因果节点数）')
    conflict_intensity = Column(Float, comment='冲突烈度（0.0-1.0）')
    foreshadow_density = Column(Float, comment='伏笔密度（每千字新埋伏笔数）')

    # ========== 节点状态快照（分析时提取，供 PM Agent 上下文使用）==========
    character_states_snapshot = Column(
        JSON,
        comment="""角色状态快照: [
        {name, status, current_state, state_updated_chapter}
    ]""",
    )
    causal_links_snapshot = Column(
        JSON,
        comment="""因果链快照（影响本章的）: [
        {cause_chapter, description, strength}
    ]""",
    )
    foreshadows_snapshot = Column(
        JSON,
        comment="""伏笔快照: [
        {title, status, plant_chapter, target_resolve_chapter, actual_resolve_chapter}
    ]""",
    )
    storylines_snapshot = Column(
        JSON,
        comment="""故事线快照: [
    ...
    def to_dict(self):
        ...
        return {
            ...
            # B方案新增指标
            'causal_density': self.causal_density or 0.0,
            'conflict_intensity': self.conflict_intensity or 0.0,
            'foreshadow_density': self.foreshadow_density or 0.0,
