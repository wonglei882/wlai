...
# sessionmaker 缓存（避免每次请求重新创建）
_session_maker_cache: dict[str, Any] = {}
_session_maker_lock = asyncio.Lock()

# 锁管理：用于保护引擎创建过程
_engine_locks: dict[str, asyncio.Lock] = {}
_cache_lock = asyncio.Lock()

# 会话统计锁（保护 _session_stats 并发安全）
_session_stats_lock = asyncio.Lock()
...
async def init_db(user_id: str = None):
    ...
    Args:
        user_id: 用户ID (已不再使用)
    """
    logger.warning('⚠️ init_db() 已弃用且无实际作用！\n   - 表结构: 由 Alembic 管理\n   - 用户配置: Settings API 自动创建\n   建议移除此调用')
...
async def get_database_stats():
    ...
    if _session_stats['active'] > settings.database_session_leak_threshold:
        ...
        stats['health']['errors'].append(f'活跃会话数 {_session_stats["active"]} 超过泄漏阈值 {settings.database_session_leak_threshold}')
    elif _session_stats['active'] > settings.database_session_max_active:
        stats['health']['status'] = 'warning'
        stats['health']['warnings'].append(f'活跃会话数 {_session_stats["active"]} 超过警告阈值 {settings.database_session_max_active}')
